...
| Deployment | Vercel | Edge network hosting with Web Analytics |
...
│   │   ├── Icon.astro              # Centralized SVG icon system (16 icons)
...
**Available icons:** `github`, `linkedin`, `instagram`, `email`, `phone`, `location`, `external-link`, `arrow-down`, `menu`, `close`, `sun`, `moon`, `calendar`, `briefcase`, `graduation`, `code`, `check`
...
- **Setup:** Replace `YOUR_WEB3FORMS_ACCESS_KEY` in Contact.astro

### Content Collections
Located in `src/content/`, schemas in `config.ts`:
- **projects** — title, description, tech[], github, featured, lang, order
- **experience** — company, role, startDate, endDate, tasks[], lang, order
...
## Important Files
- `astro.config.mjs` — Svelte, MDX, Sitemap, Vercel integrations + i18n config
