...
import {CardSwap, FadeInOnScroll, Section} from './VisualComponents';
import {SkillIcons} from './Icons';

const Skills = () => {
    const {language} = useLanguage();
    const {skills} = portfolioData[language];
    const Card = CardSwap.Card; // Acessando o sub-componente

    return (
        <Section id="skills" title={skills.title}>
            <div className="flex flex-col items-center">
                <div className="h-[400px] w-full max-w-xl flex justify-center items-center relative mb-16" style={{
                    ...
                    <CardSwap width={350} height={280}>
                        {skills.categories.map((category) => {
                            ...
                            return (
                                <div key={category.name} className="card-swap-card skill-swap-card">
                    ...
                    <div className="text-center">
                        ...
                        <div className="flex flex-wrap justify-center gap-4">
                            {skills.certifications.map((cert, index) => (
                                <div key={index}
                                     className="bg-slate-100 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 font-medium py-2 px-4 rounded-lg border border-slate-200 dark:border-slate-700">{cert}</div>
                            ))}
                        </div>
                    </div>
                </FadeInOnScroll>
