...
const Contact = () => {
    ...
    const contactItems = [
        ...
        { icon: <LinkedInIcon />, text: contact.linkedin, href: "https://www.linkedin.com/in/joaopaulosantosgo/", target: "_blank" },
    ];
    return (
                ...
                <div className="space-y-6">
                    {contactItems.map((item, index) => (
                        <FadeInOnScroll key={index} delay={index * 150}>
                            <a href={item.href} target={item.target || '_self'} rel={item.target ? 'noopener noreferrer' : ''} className="inline-flex items-center text-lg text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300 group">
                                <span className="mr-4 bg-slate-100 dark:bg-slate-800 p-3 rounded-full group-hover:bg-blue-100 dark:group-hover:bg-blue-900/50 transition-colors duration-300">
                                    {item.icon}
                                </span>
                                <span>{item.text}</span>
                            </a>
